...
class MHRGAT(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, with_relu=True,
        ...
        self.gc1 = GCNConv(nfeat, nhid, bias=True, add_self_loops=False, normalize=False)
        self.gc2 = GCNConv(nhid, nclass, bias=True, add_self_loops=False, normalize=False)
    ...
    def att_coef(self, features, adj):
        with torch.no_grad():
            ...
            sims = torch.cat((sims, torch.ones(n_total, device=sims.device).float()), dim=0)
            # normalize sims
